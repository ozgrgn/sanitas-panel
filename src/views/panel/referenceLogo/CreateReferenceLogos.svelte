<script>
  ...
  values.map((v) => {
    if (v.defaultValue) {
      referenceLogo[v.key] = { value: v.defaultValue };
    } else {
      referenceLogo[v.key] = {};
    }
  });
  ...
  const addReferenceLogo = async () => {
    let data = {};
    values.map((v) => {
      if (referenceLogo[v.key].value) {
        data[v.key] = referenceLogo[v.key]?.value;
      }
    });
    let response = await RestService.addReferenceLogo(data);
    if (response["status"]) {
      ...
      <div class="flex-auto px-4 lg:px-10 py-10 pt-0">
        ...
        <div class="flex flex-wrap">
          <div class="w-full lg:w-12/12 px-4 text-right mt-2">
            <button
              on:click={addReferenceLogo}
              disabled={!referenceLogo.lang.isValid || referenceLogo.lang.value == null}
