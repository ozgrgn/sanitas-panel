<script>
  ...
  getLangs();
  const getReferenceLogos = async () => {
    let response = await RestService.getReferenceLogos(limit, skip, lang);
    referenceLogos = response["referenceLogos"];
    console.log(referenceLogos, "referenceLogos");
...
<div class="flex flex-wrap mt-4 h-screen relative">
  <div class="w-full mb-12 px-2 lg:px-4 ">
    <button
      ...
      type="button"
      on:click={() => {
        navigate("/panel/create-referenceLogo");
      }}
    >
      <i class="fa fa-plus" />
      {$Translate("Add")}
      ...
      <div class="block w-full overflow-x-auto">
        {#if referenceLogos}
          <table class="items-center w-full bg-transparent border-collapse">
            ...
            <tbody>
              {#each referenceLogos as referenceLogo}
                <tr>
                  <td
                ...
                >
                  ...
                  >
                    <button
                      ...
                      type="button"
                      on:click={navigate(
                        `/panel/update-referenceLogo/${referenceLogo._id.toString()}`
                      )}
                    >
                      {$Translate("Edit")}
                    </button>
                    <button
                      on:click={() => deleteReferenceLogoApprove(referenceLogo._id.toString())}
                      class="bg-white text-blue-600 hover:bg-blue-600 hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
                      type="button"
                    >
                      {$Translate("Delete")}
