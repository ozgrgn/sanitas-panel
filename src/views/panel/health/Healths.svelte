<script>
  ...
  const getHealths = async () => {
    let response = await RestService.getHealths(limit, skip, lang);
    healths = response["healths"];
      ...
      <div class="rounded-t mb-0 px-4 py-3 border-0">
        <div class="flex flex-wrap items-center">
          <div class="relative w-full px-4 max-w-full flex-grow flex-1">
            <h3 class="font-semibold text-lg text-blueGray-700">Health Tourism</h3>
          </div>
        </div>
      </div>
                ...
                <tr>
                  ...
                  >
                    <button
                      ...
                      type="button"
                      on:click={navigate(
                        `/panel/update-health/${health._id.toString()}`
                      )}
                    >
                      {$Translate("Edit")}
                    </button>
                    <button
                      on:click={() => deleteHealthApprove(health._id.toString())}
                      class="bg-white text-blue-600 hover:bg-blue-600 hover:text-white border border-blue-600 rounded font-bold  text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none "
                      type="button"
                    >
                      {$Translate("Delete")}
